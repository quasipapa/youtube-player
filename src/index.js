...
( () => {
	...
	function initializePlayer( container ) {
		...
		videoContainer.className = 'ytpp-player__video';

		const controls = document.createElement( 'div' );
		controls.className = 'ytpp-player__controls';

		const previousButton = document.createElement( 'button' );
		previousButton.type = 'button';
		previousButton.className = 'ytpp-player__previous';
		previousButton.textContent = '← Vorheriges Video';

		const position = document.createElement( 'span' );
		position.className = 'ytpp-player__position';

		const nextButton = document.createElement( 'button' );
		nextButton.type = 'button';
		nextButton.className = 'ytpp-player__next';
		nextButton.textContent = 'Nächstes Video →';

		controls.append( previousButton, position, nextButton );
		container.append( videoContainer, controls );
		...
		function updatePosition() {
			...
			if (
				...
				position.textContent = `Video ${ index + 1 } von ${
					playlist.length
				}`;
			} else {
		...
		const player = new window.YT.Player( videoContainer, {
